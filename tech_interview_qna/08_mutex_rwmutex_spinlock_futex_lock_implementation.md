...
## Q080. mutex 为什么能保护临界区？

**回答：**

mutex 能保护临界区，不是因为它让代码“变成单线程”，而是因为它给一组共享状态建立了排他访问规则：同一时刻只有一个执行单元能成功持有这把锁。只要所有访问同一组共享状态的代码都遵守同一把锁，临界区里的复合操作就不会和另一个临界区交错执行。

要抓住两个关键词：共享状态和不变量。临界区保护的通常不是某一行代码，而是几份数据之间的关系。例如队列里有 `items` 和 `size`，写入时要同时改切片和计数。没有锁时，另一个线程可能看到 `items` 已经变了、`size` 还没变；有 mutex 时，其他线程必须等当前临界区结束后才能进入。
...
这个“同一把锁”很重要。如果写路径用 `muA`，读路径用 `muB`，它们互不认识，互斥关系就不存在。锁也不会自动知道哪些变量该保护，代码结构必须把锁和状态绑定起来。

mutex 还提供内存同步语义。只互斥还不够，CPU 和编译器可能重排内存访问，缓存也可能让一个核看不到另一个核刚写的值。成熟的 mutex 实现会把 unlock 做成 release，把后续 lock 做成 acquire，保证前一个临界区内完成的写入，对后一个成功拿锁的执行单元可见。

所以面试里可以这样回答：mutex 通过原子状态和等待队列控制进入资格，通过 acquire/release 语义建立可见性。它保护的是共享状态的不变量，不是某段文字上的花括号。前提是所有相关代码都按同一套锁协议访问状态。

## Q081. mutex 的加锁和解锁需要哪些内存可见性保证？

**回答：**

mutex 的内存保证可以概括成一句话：`Unlock` 要释放临界区内的写入，后续成功的 `Lock` 要获取这些写入。很多语言把这描述成 happens-before 或 synchronizes-before 关系。

如果没有这层保证，锁只会变成“排队器”。两个线程确实不会同时进入临界区，但后进入的人未必能看到前一个人的更新，这样锁就不能保护共享状态。比如线程 A 在锁内写 `x = 42`，随后 unlock；线程 B lock 成功后读 `x`。正确的 mutex 语义要求 B 能看到 A 在 unlock 前完成的写入，除非又有其他同步修改。

从实现角度看，加锁通常要有 acquire 语义，解锁通常要有 release 语义：

```text
Lock/acquire:
  后面的读写不能被重排到拿锁之前。
  当前 CPU 要看到释放锁之前发布的共享状态。

Unlock/release:
  前面的读写不能被重排到释放锁之后。
  临界区内更新要在锁变为可获取之前发布出去。
```

在更弱内存模型的硬件上，锁实现可能需要内存屏障。Linux kernel memory barriers 文档也把锁操作和 acquire/release 屏障联系起来。应用开发者一般不直接写这些屏障，但必须理解语义：锁保护的状态只能在正确加锁后访问，不能拿一个普通 bool 或普通整数去模拟锁。

Go 的说法很适合面试引用：对同一个 `sync.Mutex`，一次 `Unlock` synchronizes-before 后续某次 `Lock` 成功返回。`sync/atomic` 文档也提醒，原子操作是低层同步原语，普通场景更应使用 channel 或 `sync` 包。意思不是 atomic 不好，而是锁已经替你封装了互斥、等待和可见性。

常见错误有三类。第一，只在写路径加锁，读路径裸读，导致 data race。第二，锁内写一个指针，锁外直接读这个指针，以为“指针赋值是原子的”就安全。第三，用 `TryLock` 失败后的返回值推断数据没有变化。失败的 `TryLock` 通常不建立同步关系，不能拿它当可见性屏障。

一句话：mutex 的正确性来自互斥加可见性。没有 acquire/release，锁就不能可靠地保护内存状态。

## Q082. 用户态 spin 和内核态 sleep 的边界如何选择？

**回答：**

spin 和 sleep 的选择，本质是在赌等待时间。锁马上会释放，就在用户态自旋几轮，省掉系统调用和上下文切换；锁可能等很久，就让线程睡眠，把 CPU 让给别人。

spin 的优点是快。一次 CAS、load、pause 指令的成本远低于进入内核、挂等待队列、调度走、再被唤醒。短临界区、持锁线程正在另一个 CPU 上运行、竞争不太激烈时，spin 很划算。

spin 的问题也直接：它烧 CPU，而且会打同一条 cache line。持锁者如果被抢占、在做 I/O、等待网络、发生 page fault，等待者继续 spin 只是浪费。线程越多，锁变量所在 cache line 在多个核之间来回迁移，系统吞吐可能比直接睡眠更差。

sleep 的优点是让出 CPU。等待时间较长、线程数多于 CPU 核数、持锁路径可能阻塞时，进入内核或运行时 park 更合理。缺点是成本高：系统调用、等待队列、唤醒、上下文切换、调度延迟都会进入尾延迟。

实际锁实现常用混合策略：

```text
先尝试 fast path CAS。
失败后短暂自旋，观察 owner 是否可能很快释放。
超过阈值后进入 futex wait 或 runtime park。
解锁时按状态决定是否唤醒等待者。
```

边界通常看几个信号。临界区越短，越适合自旋；CPU 核越多、owner 正在运行，越适合自旋；线程数远大于核数、临界区里有 I/O、持锁者容易被抢占，越应该睡眠；实时系统还要考虑优先级继承，普通自旋可能放大 priority inversion。

Go、pthread、JVM 这类运行时会根据平台和竞争情况做折中。业务代码里不建议自己写无限 spin。更靠谱的做法是先用成熟 mutex，再通过 mutex profile、perf lock、调度延迟和 CPU 利用率看问题出在等待还是空转。

一句话：spin 适合短等待，sleep 适合长等待。难点不是概念，而是判断“这次等待到底短不短”。

## Q083. futex 如何把 uncontended lock 的成本留在用户态？

**回答：**

futex 的名字就是 fast userspace mutex 的缩写思路：无竞争时，锁状态完全在用户态用原子指令修改；只有真的要睡眠或唤醒时，才进入内核。
...
```text
if CAS(&state, 0, 1) succeeds:
  拿锁成功，不进内核
else:
  进入 slow path
```

无竞争时，只有一次原子操作。内核不知道这把锁存在，也没有系统调用。这就是 uncontended lock 成本低的关键。

有竞争时才需要 futex。线程 CAS 失败，发现锁已经被占用，就把 futex word 地址和期望值交给 `FUTEX_WAIT`。内核会做 compare-and-block：先检查这个地址上的值是否仍等于期望值，如果等，才把线程挂到等待队列；如果不等，说明状态已经变了，直接返回让用户态重试。

这个 compare-and-block 解决的是 lost wakeup。没有它，线程可能先看到锁被占用，准备睡眠；就在真正睡下去之前，owner 解锁并 wake；等待者随后睡着，再也没人叫醒。futex 把“检查值”和“挂起等待”放进一个原子内核动作里，避免这个窗口。

解锁时也先走用户态。如果状态显示没有等待者，直接把锁改回 0。只有发现有等待者时，才调用 `FUTEX_WAKE` 唤醒一个或多个等待线程。

所以 futex 不等于 mutex，它只是提供等待和唤醒能力。pthread mutex、Go runtime 锁、条件变量、semaphore 都可以在它之上构造自己的状态协议。协议设计得好，无竞争路径几乎就是 CAS；协议设计错了，就会出现多余 wake、丢 wake、惊群或 owner death 处理不当。

面试里一句话：futex 把高频 fast path 留给用户态原子操作，把低频 slow path 交给内核等待队列。

## Q084. 锁竞争严重时为什么会导致上下文切换和 cache line bounce？

**回答：**

锁竞争严重时，系统会同时付两笔账：线程拿不到锁要阻塞和唤醒，锁状态所在 cache line 会在 CPU 核之间来回迁移。一个是调度成本，一个是缓存一致性成本。

先看上下文切换。线程抢锁失败，如果实现判断继续 spin 不划算，就会进入内核或运行时等待队列。它从 running 变成 sleeping 或 parked。解锁方释放锁后唤醒等待者，等待者再从 blocked 变成 runnable，等调度器把它放回 CPU。每次睡眠和唤醒都可能带来系统调用、调度队列操作、寄存器上下文保存恢复、TLB/cache 热度损失。

竞争越激烈，越容易形成 lock convoy。一个慢持锁者让一批线程排队，锁释放后唤醒一个线程，那个线程跑一小段又释放，再唤醒下一个。系统看起来一直在忙，但大量时间花在等待、唤醒和切换上。

再看 cache line bounce。锁变量通常是一个小整数，但它所在的 cache line 是缓存一致性的最小单位之一。多个 CPU 核反复对同一个锁状态做 CAS、exchange、fetch-add，这条 cache line 必须在核之间获得独占写权限。CPU0 修改后，CPU1 要改，就要让 CPU0 的副本失效或迁移所有权；CPU2 再来，又迁移一次。

这就是为什么短临界区也可能慢。真正的业务代码也许只做了几条指令，但大家都在争同一把锁，锁 word 变成共享写热点。RWMutex 的 reader count、引用计数、全局统计计数也会有类似问题。

性能表现通常是：CPU 使用率高但业务吞吐不涨，mutex/futex wait 增多，perf 或 lockstat 看到 contention、wait time、cross-CPU bounce，p99 延迟抬高。处理方向不是盲目换锁，而是减少共享写热点：缩小临界区、拆分锁、按 key 分片、批量提交、减少读路径写计数，或者改成单 owner 队列模型。

一句话：严重锁竞争会让线程在调度器里排队，也会让缓存一致性协议围着同一条 cache line 忙活。

## Q085. cache coherence 为什么会影响锁性能？

**回答：**

锁性能受 cache coherence 影响，是因为锁变量和许多锁保护的数据都需要跨 CPU 核保持一致。多核 CPU 不是所有核心直接读写同一份“即时内存”；每个核心有自己的 cache。只要某个核心要写一条 cache line，就必须先拿到这条 cache line 的独占权限。

mutex 的状态位很小，但它会被频繁写：加锁时从 unlocked 改成 locked，解锁时再改回去，有等待者时还要改状态位。多个核争同一把锁时，每次 CAS 都可能让锁所在 cache line 在核之间迁移。即使 CAS 失败，它也可能触发缓存一致性流量。

临界区里的共享数据也会受影响。假设锁保护一个全局 map，线程 A 在 CPU0 上更新 map 的桶，线程 B 下一次在 CPU1 上拿锁后访问同一批 bucket。锁交接不仅交接了执行权，也经常把相关数据的 cache 热度从一个核迁到另一个核。公平锁更容易严格 handoff 到睡眠等待者，可能让 cache locality 变差；非公平锁允许刚运行的线程再次拿锁，有时吞吐更高，原因之一就是 cache 还热。

读写锁也有典型问题。理论上多个 reader 可以并发，但很多实现要在 `RLock/RUnlock` 修改 reader counter。大量 CPU 反复更新同一个计数器，哪怕读临界区没有写业务数据，计数器本身也成了 cache coherence 热点。

这也是为什么“锁竞争”不是单纯调度问题。你可能没有很多 futex sleep，CPU 也一直在跑，但吞吐就是上不去，因为核间一致性流量和原子操作序列化已经吃掉了时间。

常见优化包括：

```text
按 key 分片，减少同一 cache line 上的写者数量。
把频繁更新的计数器改成 per-CPU/per-shard 聚合。
避免把锁和无关热字段放在同一 cache line。
读多写少场景考虑 immutable snapshot、atomic pointer 或 RCU。
减少公平 handoff 带来的跨核迁移。
```

面试里可以说：锁是共享写热点，而 cache coherence 对共享写最敏感。锁越热，核之间为同一条 cache line 协调所有权的成本越高。

## Q086. false sharing 和 lock contention 有什么区别？

**回答：**

false sharing 和 lock contention 都可能表现为多核性能下降，但原因不一样。

lock contention 是多个执行单元争同一个逻辑资源。它们确实要访问同一组共享状态，必须排队或重试。比如所有请求都要更新同一个全局 map、同一个 LRU 链表、同一个队列头。这里的冲突是真冲突，业务语义上就不能随便并发。

false sharing 是没有逻辑共享，却有物理 cache line 共享。两个线程各自更新不同变量，变量恰好落在同一条 cache line 上。CPU cache coherence 以 cache line 为单位，不知道这两个变量业务上无关；一个核写 A，会让另一个核上包含 B 的同一条 cache line 失效。结果两个线程互相拖慢，但不是因为它们需要同一把锁或同一个对象。

例子很典型：

```text
struct Counters {
  uint64 requests_core0;
  uint64 requests_core1;
}
```

如果两个计数器在同一 cache line，CPU0 狂写第一个，CPU1 狂写第二个，也会互相 invalidation。这叫 false sharing。加锁热点则是大家都要改同一个 `mu.state` 或同一个全局计数器。

处理方式不同。lock contention 要减少逻辑共享：拆锁、分片、缩短临界区、换数据结构、单 owner、批处理。false sharing 要调整内存布局：padding、cache line alignment、per-CPU 数据、把热写字段分开放。把 false sharing 当 lock contention 处理，可能拆了一堆锁还没效果；把真正锁竞争当 false sharing 处理，只加 padding 也救不了。

排查时可以看几个信号。mutex profile、futex wait、block profile 很高，更像 lock contention；CPU 很高、等待不明显、perf 看到 cache misses、remote HITM、cache line invalidation，很可能有 false sharing 或原子热点。两者也可能叠在一起：锁变量本身和其他热字段放在同一 cache line，会让锁竞争更糟。

一句话：lock contention 是争同一个逻辑资源，false sharing 是无关变量误共享同一条 cache line。

## Q087. RWMutex 为什么可能出现写者饥饿？

**回答：**

RWMutex 允许多个 reader 同时进入，这本来是它的价值。但如果实现一直允许新 reader 在 writer 等待时继续插队，writer 就可能长期拿不到锁。这就是写者饥饿。

场景很简单：读请求源源不断，每个读临界区都很短，但总有至少一个 reader 在锁内。writer 到来后必须等所有 reader 退出；如果新 reader 不受限制地继续进入，reader 集合永远不归零，writer 就一直等。

早期或读者优先的读写锁容易遇到这个问题。读吞吐很好，写延迟可能没上界。配置更新、索引切换、缓存失效、元数据刷新这类写操作如果一直被 reader 淹没，系统会出现“读还能跑，写永远提交不了”的状态。

成熟实现通常会在 writer 等待后阻止新的 reader 进入。Go 的 `sync.RWMutex` 文档就说明，如果已有 goroutine 调用 `Lock` 等待写锁，后续 `RLock` 会阻塞，目的是保证 writer 最终能获得锁。这相当于从“读者优先”切到“有 writer pending 时偏向 writer”。

但这个策略也有成本。writer pending 后，新 reader 被挡住，读吞吐会下降，读延迟会上升。如果写临界区很长，所有后来的 reader 都被压在后面，p99 可能很难看。所以 RWMutex 的策略总是在读吞吐和写延迟之间折中。

还有一个常见自造饥饿：读锁升级写锁。线程持有 `RLock`，发现需要写，于是直接调用 `Lock`。如果实现不支持升级，它自己作为 reader 阻止 writer 进入，又作为 writer 等待读者清空，很容易死锁或自饥饿。Go 的 RWMutex 不支持升级，应该先释放读锁，再按设计重新获取写锁，并接受状态可能变化。

面试里可以这样说：RWMutex 的写者饥饿来自持续 reader 流量和读者优先策略。解决办法是 writer pending 后阻塞新 reader，代价是读路径尾延迟变高。

## Q088. 公平锁为什么可能牺牲吞吐？

**回答：**

公平锁把“谁先等，谁先拿”放在更高优先级。这个策略能减少饥饿和等待时间方差，但吞吐常常会下降。

原因之一是调度交接成本。非公平锁释放后，当前正在 CPU 上运行的线程或刚到的线程可能马上 CAS 成功，继续执行。公平锁则倾向于把锁交给等待队列里最早的线程。这个线程可能正在睡眠，必须被唤醒、进入 runnable 队列、等待调度，再真正运行。锁在这段时间里可能处于可交接但没有实际业务进展的状态。

第二个原因是 cache locality。刚释放锁的线程、同一 CPU 上的线程，往往还保留临界区相关数据的 cache 热度。严格 FIFO 可能把锁交给另一个 CPU 上刚被唤醒的线程，锁变量和受保护数据都要迁移。短临界区下，这个成本会盖过公平带来的好处。

第三个原因是 convoy。队列头的线程如果被抢占、优先级低、所在 CPU 忙，后面的线程即使已经准备好，也要排队等它。公平锁减少插队，但也可能把慢 waiter 的延迟传播给整个队列。

Java `ReentrantLock` 的公平模式文档就明确提醒：公平锁在竞争下倾向于给等待最久的线程，但整体吞吐可能比默认非公平模式低。Go Mutex 也不是绝对 FIFO，它在 normal mode 偏吞吐，等待超过阈值后进入 starvation mode 直接交接给队首等待者，用来限制尾延迟。

所以公平锁不是“更正确”的锁。正确性只要求互斥和可见性，公平性是调度策略。系统如果关注实时响应、避免饥饿、控制等待方差，公平更有吸引力；如果临界区短、请求量大、吞吐优先，非公平或混合策略可能更好。

一句话：公平锁买的是等待可预测性，账单是唤醒、调度和 cache locality 损失。

## Q089. priority inversion 为什么在实时系统中危险？

**回答：**

priority inversion 在实时系统里危险，是因为它会破坏高优先级任务的时间上界。实时系统关心的不是平均吞吐，而是任务必须在 deadline 前完成。高优先级任务如果被低优先级任务间接拖住，deadline 就可能失效。

经典三任务场景是：低优先级 L 持有锁；高优先级 H 需要这把锁，于是阻塞；中优先级 M 不需要这把锁，但它一直可运行，并不断抢占 L。结果 H 明明优先级最高，却要等 M 跑完、L 才有机会释放锁。Linux RT-mutex 文档把危险点叫 unbounded priority inversion，也就是等待时间没有明确上界。

普通服务器里，这种问题可能只是 p99 抖动；实时系统里，它可能直接变成控制回路错过周期、音频中断爆音、机器人控制延迟、工业设备安全逻辑失效。高优先级任务的语义就是“它比中优先级任务更该运行”，但资源依赖把这个顺序反过来了。

缓解办法通常是 priority inheritance 或 priority ceiling。priority inheritance 的做法是：当 H 等待 L 持有的锁时，临时把 L 的有效优先级提升到 H，避免 M 继续抢占 L。L 释放锁后恢复原优先级。Linux RT-mutex 和 PI-futex 就是围绕这类问题设计的。

但优先级继承不等于万能。它不能让长临界区变短，不能修死锁，也不能处理所有资源依赖。实时系统仍要控制持锁时间，避免持锁 I/O，减少锁嵌套，分析最坏情况阻塞时间。否则即使用了 PI，H 仍然要等 L 把临界区跑完。

面试里可以这样回答：priority inversion 的危险不在“优先级看起来反了”，而在高优先级任务等待时间失去上界。实时系统最怕这个。

## Q090. 死锁、活锁、饥饿分别是什么？

**回答：**

这三个词都表示并发系统前进性出问题，但形状不一样。

死锁是大家互相等待，没人能继续。最典型的是两个线程反向拿锁：线程 A 持有 `L1` 等 `L2`，线程 B 持有 `L2` 等 `L1`。等待图里形成环，除非外部干预，否则相关线程不会前进。死锁通常 CPU 不高，请求卡住，goroutine/thread dump 能看到一批执行单元阻塞在锁、channel、条件变量或连接池上。

活锁是大家都在动，但没有有效进展。比如两个线程都很礼貌，发现冲突就同时退让，然后又同时重试，再同时退让。日志刷得很快，CPU 也不低，但真正提交的业务操作很少。乐观锁在高冲突下反复 CAS 失败重试，也可能接近活锁。

饥饿是某个执行单元长期得不到资源，但系统整体还能前进。比如非公平锁下一直有新线程插队，某个老 waiter 等很久；读者优先 RWMutex 下 writer 一直拿不到写锁；线程池里低优先级任务总被高优先级任务挤掉。饥饿不要求等待图成环，也不要求所有人都停住。

可以用表格感理解：

```text
死锁：没人能走，因为等待关系成环。
活锁：大家都在走，但一直绕圈，业务不前进。
饥饿：别人能走，某个倒霉的执行单元一直走不了。
```

排查方法也不同。死锁要画等待图，找环；活锁要看重试率、失败原因、退避策略和成功吞吐；饥饿要看等待时间分布、公平性策略、优先级、队列头是否长期换不到资源。

面试里不要把所有“卡住”都叫死锁。很多线上问题其实是严重锁竞争、外部 I/O 阻塞、线程池耗尽或饥饿。先判断系统是否整体还能前进，再看是否存在等待环。

## Q091. 如何通过锁顺序、超时、try-lock 或 lock hierarchy 降低死锁风险？

**回答：**

降低死锁风险的核心是破坏循环等待。锁顺序、超时、try-lock 和 lock hierarchy 都是在不同层面做这件事。

锁顺序是最常用的办法。给所有可能嵌套获取的锁定义全局顺序，代码只能按这个顺序加锁，通常反向释放。账户转账不能按调用参数顺序锁 `from` 再锁 `to`，而应该按 account id 排序后加锁。动态锁集合也要先去重、排序，再逐个获取。

lock hierarchy 是把锁顺序制度化。比如规定：全局 registry 锁在 shard 锁之前，shard 锁在 item 锁之前；调度器锁在 worker 锁之前；元数据锁在数据块锁之前。层级要写进注释、设计文档或封装 API，不能只靠口头约定。Linux lockdep 这类工具也是通过 lock class 和依赖关系来发现反向顺序。

try-lock 的作用是避免“拿着 A 死等 B”。如果拿不到 B，就释放 A，退避后重试，或者走降级路径。它适合偶发多锁组合，但不能滥用。无退避的 try-lock 循环可能把死锁换成活锁；失败路径如果没有业务语义，也只是把 bug 藏起来。

超时能把无限等待变成可观测失败。比如拿锁超过 200ms 就记录持锁者、栈、资源 id，返回错误或触发降级。超时不是自动修复，尤其本地 mutex 很多语言没有带 timeout 的标准接口；但在分布式锁、数据库锁、RPC 资源锁里，超时是避免永久挂死的重要边界。

还要减少持锁期间的外部调用。持锁做 I/O、调用回调、等待 channel、访问数据库，都可能把新的等待关系引进来，绕过原本清楚的锁层级。

面试里可以这样收束：先用全局顺序和 lock hierarchy 消灭结构性环；对少数动态组合用 try-lock 和退避；对跨系统等待加超时和观测；持锁期间不调用不可控代码。

## Q092. 当锁成为瓶颈时，sharding、copy-on-write、RCU、lock-free 哪些方向可选？

**回答：**

锁成为瓶颈时，先别急着写无锁结构。要先判断瓶颈来自哪里：是不是所有请求都争同一个状态，读多写少，临界区太长，还是锁内做了慢操作。不同原因对应不同方案。

sharding 是最常用也最稳的方向。把一把全局锁拆成多把 shard 锁，按 key、tenant、bucket、CPU 或连接分片。不同 shard 并发执行，同一 shard 内仍用普通锁保证正确性。它适合 map、cache、session、统计计数、队列分组。缺点是跨 shard 操作要排序，否则会引入新的死锁。

copy-on-write 适合读多写少、对象可整体替换的场景。写者复制一份新结构，修改后用锁或 atomic pointer 发布；读者读不可变快照，减少读锁甚至不需要读锁。配置、路由表、规则表、只读索引很适合。缺点是写放大和内存峰值，频繁写会很贵。

RCU 更进一步：读者几乎不阻塞，写者发布新版本，旧版本等所有旧读者退出 grace period 后再回收。Linux 内核大量使用 RCU，因为它适合读极多、读路径不能被锁拖慢的场景。代价是实现和内存回收复杂，读者不能随便阻塞，更新语义也不是普通 mutex 那么直观。

lock-free 用 CAS、fetch-add、atomic pointer 等原子操作，让系统整体在某个线程停顿时仍能前进。它适合计数器、栈、队列、环形缓冲、引用发布这类可找到清晰线性化点的数据结构。代价是 ABA、memory ordering、内存回收、重试风暴和测试复杂度。

还有一些更朴素的选择：缩小临界区，把 I/O 移出锁外，批量更新，单 owner 线程处理状态，使用现成并发容器，改数据模型减少共享写。

选择顺序通常是：先减少锁内工作，再分片；读多写少考虑 snapshot/COW/RCU；只有 profile 证明锁仍是核心瓶颈，并且团队能承担验证成本时，再考虑 lock-free。

一句话：锁瓶颈不等于无锁需求。多数工程问题用分片和不可变快照就能解决。

## Q093. CAS 为什么可以替代某些锁？

**回答：**

CAS 可以替代某些锁，是因为它把“检查旧值”和“写入新值”合成一个硬件原子操作。全称 compare-and-swap 或 compare-and-set：如果内存当前位置仍然等于我刚才看到的旧值，就把它改成新值；否则失败。
...
loop:
  old = load(counter)
  next = old + 1
  if CAS(counter, old, next) succeeds:
    return next
  else:
    retry
```

这里没有 mutex，但更新仍然不会丢。两个线程同时读到 `old=10`，只有一个 CAS 能把 10 改成 11；另一个失败后重新读取，再改成 12。CAS 成功的那一瞬间就是这个操作的提交点。

它能替代锁的前提是状态变更可以表达成“基于一个可验证旧状态生成新状态”。单个指针发布、状态机从 `INIT` 到 `RUNNING`、引用计数、无锁栈 push/pop、一次性初始化标记，都比较适合。

CAS 的好处是无竞争时很轻，失败时不用睡眠，某个线程暂停也不会天然阻塞所有人。它避免了锁 owner 被抢占后其他线程全等着的问题。

但 CAS 不是“免费并发”。高竞争下，很多线程会反复失败重试，CPU 浪费不比锁小；状态跨多个字段时，单个 CAS 不够，需要打包、双字 CAS 或更复杂协议；失败重试的函数必须没有不可撤销副作用；内存排序和对象回收也要处理清楚。

所以面试里可以说：CAS 可以把某些短小、可重试、可用单点状态描述的临界区，压缩成一个原子提交动作。它替代的是一部分锁，不是所有锁。

## Q094. CAS 为什么会遇到 ABA 问题？

**回答：**

ABA 问题是 CAS 的旧值检查太窄：它只知道“现在是不是又等于 A”，不知道中间是否从 A 变成 B，再变回 A。

一个无锁栈例子最容易说明。线程 T1 读取栈顶是节点 A，A 的 next 是 B，准备 CAS 把 top 从 A 改成 B。它被抢占。线程 T2 运行，把 A pop 掉，又把 B pop 掉，随后可能把 A 重新 push 回来。现在 top 又是 A。T1 恢复后执行 CAS(top, A, B)，CAS 成功，因为 top 确实等于 A。问题是这个 B 已经不再是原来语义下的 A.next，甚至可能已经被释放或复用。

从 CAS 的角度看没错，从数据结构角度看坏了。它验证的是地址或数值相等，不验证对象历史。

常见解决办法有几类：

```text
版本号/tagged pointer:
  指针和版本一起 CAS，每次修改版本递增。

hazard pointer / epoch reclamation:
  保证别的线程还可能持有引用时，节点不会被释放复用。

引用计数或 pin:
  读取节点后先保护它，再继续操作。

避免复用敏感节点:
  某些场景延迟回收或使用 GC，降低 ABA 破坏性。
```

有 GC 的语言也不是完全免疫。GC 可以减少“内存被释放后复用”的危险，但逻辑 ABA 仍然存在：状态 A->B->A 后，CAS 仍然看不出中间发生过变化。比如一个状态位从 idle 到 running 又回 idle，等待者如果只看 idle，可能误判“没人动过”。

面试里可以这样回答：CAS 只比较当前值，不比较变化历史；只要值能回到旧值，就可能发生 ABA。解决要么扩大比较内容，加版本；要么保证旧引用不会被危险复用；要么重新设计状态机，让“回到 A”也能被识别。

## Q095. memory ordering 为什么是无锁编程的难点？

**回答：**

无锁编程难，很多时候不是 CAS 本身难，而是 memory ordering 难。原子操作能保证某个变量的读改写不被撕裂，但它不自动说明其他普通读写什么时候对别的 CPU 可见。
...
我们想要的是：reader 看到 `ptr` 非空时，也能看到 writer 对 `obj.field` 的初始化。这个关系需要 release/acquire。writer 发布指针时用 release，reader 读取指针时用 acquire。否则在弱内存模型上，reader 可能先看到指针，再看到未初始化或旧的字段。

锁把这套规则封装好了：unlock release，lock acquire。无锁结构要自己给每个原子操作选 ordering。太弱会错，太强会慢；更糟的是，错通常很难复现，在 x86 上没问题，到 ARM、POWER 或编译器优化后才暴露。

memory ordering 还和线性化点、对象生命周期绑在一起。一个队列的 enqueue 可能先写节点内容，再把节点链接进队列。链接 CAS 成功是线性化点，但节点内容必须在线性化点前对消费者可见。pop 成功后，什么时候能释放节点，也要保证没有其他线程还在读。这里就牵涉 acquire/release、hazard pointer、epoch、引用保护。

Go 的 `sync/atomic` 文档把原子操作定义成顺序一致语义，这减轻了应用层选择 ordering 的负担，但仍然要求你正确设计协议。C/C++、Rust、Linux kernel 里可以选择 relaxed、acquire、release、acq_rel、seq_cst，表达能力更强，错误空间也更大。

面试里可以说：无锁不是“把锁换成 CAS”。你还要证明普通数据写入在发布前完成，消费者读取在获取后发生，节点不会过早回收，并且每个原子操作的顺序足以支撑这个证明。

## Q096. lock-free 是否意味着没有等待？

**回答：**

lock-free 不意味着没有等待。它的意思是系统整体不会因为某个线程停住而完全停住；总有某个线程能在有限步内取得进展。单个线程仍然可能一直失败、一直重试，甚至饥饿。
...
如果 CAS 失败，说明通常有别的线程成功修改了 `x`。从系统整体看，有人前进了，所以这是 lock-free 味道。但某个倒霉线程可能每次准备 CAS 时都被别人抢先，它自己很久都成功不了。

这和 mutex 的阻塞不同。mutex 里，持锁线程被抢占或崩溃时，其他等待者可能都卡在这把锁上。lock-free 算法通常不依赖一个特定 owner 释放锁，所以一个线程停住不会天然阻塞全体。但它仍然有等待，只是等待形式变成重试、竞争、退避和内存回收等待。

高竞争下，lock-free 也可能很慢。大量线程围绕同一个原子变量 CAS，失败重试会制造 cache line bounce 和 CPU 空转。实际吞吐可能不如一个会睡眠的 mutex。lock-free 保证的是一种进展性质，不保证低延迟，不保证公平，也不保证每个线程都能按时完成。
...
面试里一句话：lock-free 的 “free” 是不依赖锁 owner 的阻塞，不是没有等待、没有重试、没有尾延迟。

## Q097. wait-free 为什么比 lock-free 更强？

**回答：**

wait-free 比 lock-free 更强，因为它要求每个线程的每个操作都能在有限步内完成。lock-free 只要求系统整体有人前进，wait-free 要求每个参与者都不会被饿死。

用 CAS 计数器举例。普通 CAS 循环通常可以说接近 lock-free：如果你失败了，往往说明别人成功了，系统整体在前进。但某个线程可能一直失败。它没有 per-thread 完成上界，所以不是 wait-free。

wait-free 算法要给每个操作一个明确步数上界，不能依赖“总会轮到我”。这通常需要 helping 机制：线程发现别人的操作卡在中间，会帮它完成；或者每个线程有独立槽位，聚合读取时把所有槽位合并；或者用更复杂的数据结构让每个线程进展互不阻塞。

代价也明显。wait-free 算法往往更复杂，占更多内存，常数成本更高，证明更难。很多工程系统宁愿接受 lock-free 或普通锁，也不追求 wait-free，因为它们更关心平均吞吐、p99、可维护性和故障可诊断，而不是严格的每线程步数上界。

wait-free 适合对单个参与者延迟有硬要求的场景，比如实时系统、某些信号处理路径、不能被其他线程进度影响的低层组件。普通后端服务里，成熟 mutex、分片锁、无锁队列、RCU 或 actor 模型通常已经足够。

面试里可以这样说：lock-free 防止“一个线程停住拖死所有人”；wait-free 进一步防止“某个线程一直被别人抢先”。强度更高，工程成本也更高。

## Q098. 线性化点在并发数据结构中如何定义？

**回答：**

线性化点是一个并发操作在逻辑上“瞬间生效”的位置。并发操作可能执行很多步，和其他线程交错，但如果这个数据结构是线性化的，每个操作都应该能找到一个位于调用和返回之间的点，使得所有操作按这些点排序后，结果等价于某个合法的顺序执行。

它不是随便挑一行。线性化点必须解释对象状态为什么在那一刻改变，后续观察为什么符合顺序语义。

简单例子：mutex 保护的 map 里，`Put(k,v)` 的线性化点通常在持锁临界区内真正写入 map 的那一刻，或者从外部看可以视为整个锁内操作原子生效。因为锁让其他访问者不能在中间观察到半更新状态。

CAS 栈里，`Push` 的线性化点通常是把 top 从旧节点 CAS 到新节点成功的那一瞬间。CAS 成功前，新节点还不属于栈；CAS 成功后，其他线程就可能看到它。失败的 CAS 不是线性化点，只是一次未提交尝试。

队列、哈希表、RCU 结构会更麻烦。有些操作的线性化点取决于竞争结果；有些读操作的线性化点可能在读取某个版本指针时；有些 contains 操作需要证明它读到的节点在某个时间点确实在集合里。线性化点越模糊，算法越难验证。

为什么面试要讲这个？因为并发数据结构不能只说“最后结果差不多对”。你要说明每个操作何时对外可见，失败操作何时判定失败，读操作读到的是哪个瞬间的状态。这关系到测试、推理和 bug 定位。

一句话：线性化点是并发操作的原子生效点。找得到并能证明它，才谈得上线性化的数据结构。

## Q099. 为什么工程上常建议优先使用成熟锁而不是手写无锁结构？

**回答：**

工程上建议优先用成熟锁，是因为锁的语义简单、实现经过验证、出问题时容易观测。手写无锁结构看起来很酷，但正确性成本很高，收益也经常被高估。

成熟 mutex 或 RWMutex 已经处理了大量细节：互斥、等待队列、调度交接、内存屏障、竞争退化、饥饿控制、race detector 友好性、profile 观测。你用它时，主要证明的是“锁保护了正确状态，锁顺序没有问题”。这已经不轻松，但至少边界清楚。

手写无锁结构要证明更多东西：CAS 循环的线性化点在哪里，失败重试是否有副作用，ABA 怎么处理，节点什么时候能释放，memory ordering 是否足够，读者会不会看到半初始化对象，高竞争下是否活锁，某个线程是否可能饥饿。测试也难，因为很多 bug 只在特定 CPU、特定编译优化、特定调度交错下出现。

性能上也不是无锁必胜。高竞争 CAS 会造成 cache line bounce，失败重试会烧 CPU；一个会睡眠的 mutex 反而可能让系统吞吐更稳。很多锁实现已经是混合策略：无竞争 fast path 是原子操作，竞争时才 park。你手写的无锁结构未必比它快。
...
先用普通锁写出正确版本。
用 profile 证明锁是瓶颈。
先缩小临界区、移出 I/O、做 sharding 或 copy-on-write。
仍然不够时，优先选成熟并发容器或 runtime 提供的原语。
最后才考虑自研无锁，并配套模型测试、stress、race、benchmark 和代码审查。
```

面试里可以这样说：无锁是工具，不是技术成熟度证明。能用简单锁把不变量讲清楚，通常比写一个没人敢改的 CAS 结构更工程化。
