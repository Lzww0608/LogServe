# LogServe Resume Notes

## 中文项目描述

LogServe 是一个基于 shared log 的 AI runtime，主要面向 workflow、actor 和 LLM serving。项目包含 Go 控制面、append-only log 服务、worker 执行器、Python SDK、workflow DAG 调度、有状态 actor 恢复、模型缓存感知调度、checkpoint cache、故障注入、benchmark 和 dashboard。
...
- 补齐系统测试和评估工具：fault injection、workflow/task/actor/LLM benchmark、snapshot/locality/replay ablation、dashboard snapshot、backpressure 和单机 Ubuntu 实验脚本。
...
- Shared log benchmark 中，20,000 条记录、16 streams、256-byte payload 下，batch/interval fsync append throughput 约为 239k/266k records/s，always fsync 约为 1.7k records/s。
- Fault injection 中 worker kill recovery、queue redelivery、control restart probe 均通过；dashboard snapshot 记录 task、workflow、actor、worker 和 model cache 当前状态。

## 英文简历表述

- Built LogServe, a shared-log-based AI runtime in Go and Python, covering workflow DAG execution, stateful actors, LLM serving, model-cache-aware scheduling, replay, checkpointing, fault injection, benchmarking, and dashboard snapshots.
...
- Log retention 当前是 logical trim，不物理删除 segment；compactable bytes 用于估算可 compact 的空间。
- Kubernetes 部署文件保留为运行示例，实验结论仍以单机脚本结果为准。
