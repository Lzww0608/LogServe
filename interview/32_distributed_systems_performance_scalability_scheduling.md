...
还有一种选择是承认这个对象已经不像 actor，更像数据库里的热点行。此时可以把状态迁到专门的存储，比如带事务的 KV 或数据库，把 actor 变成访问这个存储的封装。面试里我会明确说：actor 热点不能靠“给同一个 actor 加更多 worker”解决。只要要保持严格顺序，横向扩容就必须改变状态建模方式。
